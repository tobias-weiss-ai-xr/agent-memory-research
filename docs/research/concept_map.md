# Concept Map — Knowledge Graph of the Corpus

**Generated:** 25 concepts, 125 relationships from 10162 papers. Edges = co-occurrence (shared papers).

## Top concepts

| Concept | Kind | Papers |
|----------|------|--------|
| Factual | taxonomy | 4379 |
| Working | taxonomy | 4035 |
| framework | curated | 2849 |
| Experiential | taxonomy | 1740 |
| survey | curated | 1113 |
| benchmark | curated | 996 |
| dataset | curated | 861 |
| quantitative | curated | 217 |
| taxonomy | curated | 163 |
| qualitative | curated | 155 |
| experiment | curated | 153 |
| case study | curated | 107 |
| systematic review | curated | 87 |
| reproducibility | curated | 67 |
| literature review | curated | 46 |
| interview | curated | 21 |
| mixed methods | curated | 11 |
| research gap | curated | 11 |
| open access | curated | 10 |
| Method | taxonomy | 8 |
| peer review | curated | 7 |
| triangulation | curated | 6 |
| grounded theory | curated | 3 |
| Survey | taxonomy | 0 |
| meta analysis | curated | 0 |

## Relationships (top concepts → related concepts)

### Factual

- framework — 1362 shared paper(s)
- survey — 1034 shared paper(s)
- benchmark — 502 shared paper(s)
- dataset — 293 shared paper(s)
- quantitative — 137 shared paper(s)
- taxonomy — 114 shared paper(s)
- qualitative — 93 shared paper(s)
- case study — 59 shared paper(s)

### Working

- framework — 817 shared paper(s)
- dataset — 453 shared paper(s)
- benchmark — 271 shared paper(s)
- experiment — 57 shared paper(s)
- quantitative — 57 shared paper(s)
- survey — 51 shared paper(s)
- systematic review — 45 shared paper(s)
- qualitative — 38 shared paper(s)

### framework

- Factual — 1362 shared paper(s)
- Working — 817 shared paper(s)
- Experiential — 664 shared paper(s)
- benchmark — 412 shared paper(s)
- dataset — 298 shared paper(s)
- survey — 157 shared paper(s)
- quantitative — 64 shared paper(s)
- taxonomy — 49 shared paper(s)

### Experiential

- framework — 664 shared paper(s)
- benchmark — 221 shared paper(s)
- dataset — 115 shared paper(s)
- experiment — 42 shared paper(s)
- survey — 28 shared paper(s)
- qualitative — 24 shared paper(s)
- taxonomy — 24 shared paper(s)
- quantitative — 23 shared paper(s)

### survey

- Factual — 1034 shared paper(s)
- framework — 157 shared paper(s)
- quantitative — 86 shared paper(s)
- taxonomy — 83 shared paper(s)
- Working — 51 shared paper(s)
- qualitative — 50 shared paper(s)
- dataset — 46 shared paper(s)
- benchmark — 36 shared paper(s)

### benchmark

- Factual — 502 shared paper(s)
- framework — 412 shared paper(s)
- Working — 271 shared paper(s)
- Experiential — 221 shared paper(s)
- dataset — 159 shared paper(s)
- survey — 36 shared paper(s)
- taxonomy — 28 shared paper(s)
- reproducibility — 24 shared paper(s)

### dataset

- Working — 453 shared paper(s)
- framework — 298 shared paper(s)
- Factual — 293 shared paper(s)
- benchmark — 159 shared paper(s)
- Experiential — 115 shared paper(s)
- survey — 46 shared paper(s)
- experiment — 23 shared paper(s)
- quantitative — 20 shared paper(s)

### quantitative

- Factual — 137 shared paper(s)
- survey — 86 shared paper(s)
- qualitative — 64 shared paper(s)
- framework — 64 shared paper(s)
- Working — 57 shared paper(s)
- Experiential — 23 shared paper(s)
- dataset — 20 shared paper(s)
- benchmark — 13 shared paper(s)

### taxonomy

- Factual — 114 shared paper(s)
- survey — 83 shared paper(s)
- framework — 49 shared paper(s)
- benchmark — 28 shared paper(s)
- Working — 25 shared paper(s)
- Experiential — 24 shared paper(s)
- systematic review — 7 shared paper(s)
- quantitative — 6 shared paper(s)

### qualitative

- Factual — 93 shared paper(s)
- quantitative — 64 shared paper(s)
- survey — 50 shared paper(s)
- framework — 48 shared paper(s)
- Working — 38 shared paper(s)
- Experiential — 24 shared paper(s)
- dataset — 15 shared paper(s)
- benchmark — 14 shared paper(s)

### experiment

- Working — 57 shared paper(s)
- Factual — 54 shared paper(s)
- Experiential — 42 shared paper(s)
- framework — 32 shared paper(s)
- dataset — 23 shared paper(s)
- benchmark — 16 shared paper(s)
- survey — 14 shared paper(s)
- quantitative — 10 shared paper(s)

### case study

- Factual — 59 shared paper(s)
- framework — 37 shared paper(s)
- Working — 33 shared paper(s)
- survey — 19 shared paper(s)
- Experiential — 15 shared paper(s)
- dataset — 9 shared paper(s)
- benchmark — 8 shared paper(s)
- qualitative — 5 shared paper(s)

### systematic review

- Working — 45 shared paper(s)
- Factual — 30 shared paper(s)
- survey — 16 shared paper(s)
- framework — 13 shared paper(s)
- Experiential — 12 shared paper(s)
- qualitative — 8 shared paper(s)
- taxonomy — 7 shared paper(s)
- literature review — 7 shared paper(s)

### reproducibility

- Factual — 37 shared paper(s)
- framework — 25 shared paper(s)
- benchmark — 24 shared paper(s)
- Working — 16 shared paper(s)
- Experiential — 14 shared paper(s)
- dataset — 13 shared paper(s)
- survey — 7 shared paper(s)
- taxonomy — 4 shared paper(s)

### literature review

- Factual — 24 shared paper(s)
- survey — 15 shared paper(s)
- Working — 14 shared paper(s)
- framework — 10 shared paper(s)
- Experiential — 8 shared paper(s)
- systematic review — 7 shared paper(s)
- taxonomy — 5 shared paper(s)
- dataset — 5 shared paper(s)

### interview

- Factual — 15 shared paper(s)
- survey — 11 shared paper(s)
- qualitative — 5 shared paper(s)
- framework — 4 shared paper(s)
- Experiential — 4 shared paper(s)
- quantitative — 2 shared paper(s)
- Working — 2 shared paper(s)

### mixed methods

- Factual — 9 shared paper(s)
- qualitative — 6 shared paper(s)
- quantitative — 4 shared paper(s)
- survey — 3 shared paper(s)

### research gap

- Working — 7 shared paper(s)
- framework — 5 shared paper(s)
- dataset — 5 shared paper(s)
- Factual — 3 shared paper(s)
- quantitative — 3 shared paper(s)
- qualitative — 3 shared paper(s)
- survey — 2 shared paper(s)
- literature review — 2 shared paper(s)

### open access

- Factual — 7 shared paper(s)
- dataset — 4 shared paper(s)
- framework — 3 shared paper(s)
- survey — 3 shared paper(s)
- Working — 2 shared paper(s)

### Method

- framework — 6 shared paper(s)
- benchmark — 2 shared paper(s)

### peer review

- Factual — 5 shared paper(s)
- survey — 2 shared paper(s)

### triangulation

- Factual — 5 shared paper(s)
- survey — 3 shared paper(s)
- qualitative — 2 shared paper(s)
- quantitative — 2 shared paper(s)

### grounded theory

- Factual — 2 shared paper(s)


Generated by `tools/relate_concepts.py` — do not edit by hand.