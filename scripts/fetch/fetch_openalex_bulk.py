#!/usr/bin/env python3
"""Bulk-fetch agent-memory-research papers from OpenAlex, one request per category.

Generated for the agent-memory-research taxonomy. Uses OpenAlex cursor pagination with
...
CATEGORY_TERMS = [
    ('factual', 'memory large language models factual knowledge'),
    ('experiential', 'episodic memory agents experience'),
    ('working', 'working memory cognitive models'),
]

# Subcategory keyword rules (repo taxonomy)
SUBCAT_KEYWORDS = [
    ('token-level', ['context', 'token', 'window', 'scratchpad', 'prompt']),
    ('parametric', ['parametric', 'weights', 'fine-tun', 'knowledge']),
    ('latent', ['latent', 'representation', 'hidden', 'embedding']),
]
...
def main():
    parser = argparse.ArgumentParser(description="Bulk-fetch agent-memory-research papers from OpenAlex per category")
