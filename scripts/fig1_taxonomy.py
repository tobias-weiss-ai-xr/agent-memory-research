...
fig, axes = plt.subplots(1, 2, figsize=(10, 3.5), gridspec_kw={"width_ratios": [1.3, 1]})
...
for j, sl in enumerate(sub_labels):
    ax.text(start_x + sum(col_widths[: j + 1]) + col_widths[j + 1] / 2, start_y, sl,
            fontsize=8, ha="center", va="bottom", fontweight="bold")

# Rows
for i, (cat, cl) in enumerate(zip(cats, cat_labels)):
    y = start_y - (i + 1) * row_height
    ax.text(start_x + col_widths[0] / 2, y + row_height / 2, cl,
            fontsize=9, ha="center", va="center", fontweight="bold")
    for j, sub in enumerate(subs):
        cx = start_x + sum(col_widths[: j + 1]) + col_widths[j + 1] / 2
        cnt = counts.get((cat, sub), 0)
        ax.text(cx, y + row_height / 2, str(cnt), fontsize=11, ha="center", va="center",
                color="darkblue", fontweight="bold")

# Total row
y_tot = start_y - 4 * row_height
ax.text(start_x + col_widths[0] / 2, y_tot + row_height / 2, "Total",
        fontsize=9, ha="center", va="center", fontweight="bold")
for j, sub in enumerate(subs):
    cx = start_x + sum(col_widths[: j + 1]) + col_widths[j + 1] / 2
    total = sum(counts.get((c, sub), 0) for c in cats)
    ax.text(cx, y_tot + row_height / 2, str(total), fontsize=11, ha="center", va="center",
            color="darkred", fontweight="bold")

ax.set_xlim(0, 1.1)
ax.set_ylim(0, 1)
ax.set_title("Original 3x3 Taxonomy (Paper Counts)", fontsize=10, fontweight="bold", pad=5)
...
for idx, (dim_name, levels) in enumerate(dims):
    ...
    for j, lv in enumerate(levels):
        ...
        ax2.add_patch(mpatches.FancyBboxPatch(
            (x - 0.06, y - 0.08), 0.14, 0.16,
            boxstyle="round,pad=0.02", facecolor="lightsteelblue", edgecolor="steelblue", lw=0.8))
...
plt.tight_layout()
out = base / "paper/figures/fig1_taxonomy.pdf"
